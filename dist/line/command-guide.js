...
exports.buildCommandKeywordGuidance = exports.getCommandsForCategory = exports.parseGuideCategoryKey = exports.isGuideCommand = exports.GUIDE_CATEGORY_NOTES = exports.GUIDE_CATEGORY_LABELS = exports.GUIDE_CATEGORY_ORDER = void 0;
const COMMAND_SPECS = [
...
exports.GUIDE_CATEGORY_ORDER = ['basics', 'commerce', 'directory', 'catalog', 'reporting', 'groupBuy', 'admin', 'account'];
exports.GUIDE_CATEGORY_LABELS = {
...
const scoreSimilarity = (input, candidate) => {
    const maxLen = Math.max(input.length, candidate.length) || 1;
    const distance = levenshtein(input, candidate);
    return 1 - distance / maxLen;
};
const allCommandKeys = () => COMMAND_SPECS.flatMap(spec => [spec.key, ...(spec.aliases || [])]).map(normalize);
const topMatches = (intent) => {
...
exports.isGuideCommand = isGuideCommand;
...
const parseGuideCategoryKey = (input) => {
    const upper = input.trim().toUpperCase();
    if (!upper.startsWith('GUIDE '))
        return null;
    const token = upper.slice('GUIDE '.length).trim();
    const match = exports.GUIDE_CATEGORY_ORDER.find(key => key.toUpperCase() === token);
    return match || null;
};
exports.parseGuideCategoryKey = parseGuideCategoryKey;
const getCommandsForCategory = (category) => COMMAND_SPECS.filter(spec => spec.category === category && spec.key !== 'GUIDE').map(spec => ({ key: spec.key, example: spec.examples[0] }));
exports.getCommandsForCategory = getCommandsForCategory;
const buildCommandKeywordGuidance = (input, language, agentName) => {
...
exports.buildCommandKeywordGuidance = buildCommandKeywordGuidance;
