...
| **Overall architecture / design quality** | 10/10 (was 8) | The ERP boundary is a real adapter+registry, fail-closed for unimplemented providers, wired into every commerce/directory/catalog/reporting handler; `CLAUDE.md` documents it. Firestore/Odoo/`src/index.ts`/**and now `templates.ts`** are all barrel-split into domain modules — the 836-line HTTP entrypoint is a ~35-line orchestrator (Track B3), and the 975-line Flex-builder file is now 6 domain files + a barrel (Track B4, templates half), both verified via live local smoke tests, not just build/test. `salesTier` is a real additive layer on top of the protected chain, never replacing it. Track A3's audit sweep is done too. Remaining items are both deliberate, deferred choices, not defects: a completed-but-unwired command-registry/policy foundation (`src/ux/`, `command-policy.ts`) for a future menu-projection feature (Track C2), and `src/demo/page.ts`'s asset extraction (the smaller, not-yet-started half of Track B4). | [ ] Track C2 (registry-to-menu projection), whenever menu-driven navigation off the registry is actually wanted. [ ] `src/demo/page.ts` asset extraction, low priority. |
...
(deliberately deferred, no third language yet), and the remaining Track
B4/C2/C3 items in `STAGED_IMPLEMENTATION_BACKLOG.md` (`src/demo/page.ts`
asset extraction, menu projection, a future-ERP-provider spike — all
intentionally not started). Nothing left is a speculative or
