...
export const registerVerifyRoutes = (app: Express): void => {
    app.get('/verify/odoo', verifyLinkLimiter, async (req, res) => {
        ...
        // LINE in-app browser blocks `line://` and `%40` in https://line.me/R/ti/p/@id.
        const returnLink = oaChatDeepLink(result.channelId || DEFAULT_CHANNEL_ID);
        const cta = result.ok ? 'Return to chat' : 'Open LINE';
